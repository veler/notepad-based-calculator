...
Param(
    ...
    [string]$MSBuildPath,
    [Parameter()]
    [string]$NuGetVersion='6.2.1'
)

if ($Path.EndsWith('.sln')) {
    Write-Verbose "Restoring NuGet packages for $Path with verbosity $Verbosity"
    Invoke-Expression '& "$MSBuildPath" $Path /t:restore /p:Configuration=Release /p:platform="Any CPU" /v:$Verbosity'
    Invoke-Expression '& "$MSBuildPath" $Path /t:restore /p:Configuration=Debug /p:platform="Any CPU" /v:$Verbosity'
